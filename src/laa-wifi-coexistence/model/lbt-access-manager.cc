...
LbtAccessManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LbtAccessManager")
    ...
    .AddAttribute ("DeferTime", "TimeInterval to defer during CCA",
                   ...
                   MakeTimeChecker ())
    .AddAttribute ("Table", "Q table",
            TimeValue (MilliSeconds (10)),
            MakeTimeAccessor (&LbtAccessManager::m_table),
          MakeTimeChecker ())
...
LbtAccessManager::SetWifiPhy (Ptr<SpectrumWifiPhy> phy)
{
  ...
  for(int i = 0; i < 500; ++i) {
    x = x + 23000000.0;
    qtable[i][0] = (Time) (x/700.0);

    qtable[i][1] = (Time) 0;

    }
  s0 = GetCurrentBackoffCount();
}
...
void LbtAccessManager::SetGrant()
{
   ...
   ChannelAccessManager::SetGrantDuration (MachineLearningSet());
...
Time LbtAccessManager:: MachineLearningSet ()
{
...
*/
    ...
    random =(rand()%430);
      if(counter > 900){
        ...
        if(qtable[random][1] != (Time) 0){
          a = qtable[random][1];
        //  std::cout << qtable[random][1] << std::endl;
        }else{
          a = qtable[random][0];
        }
        ...
        a = qtable[random][0];
        //std::cout << qtable[random][0] << std::endl;
      }

      //Pick a random txTime from Q table
      //std::cout << qtable[random][0]/(100000) << std::endl;
      m_txop = a;
      return a;
...
//Get new result
void LbtAccessManager:: MachineLearningGet (){

      s1 = GetCurrentBackoffCount();
      //Time temp = Q[1][1];//Seconds (s1);
      //s1 = Q[0][0];
...
//  }
  /*for(int i = 0; i < 10; ++i) {
    std::cout << Q[i][0].As (Time::MS) << std::endl;
  }
  */
}
