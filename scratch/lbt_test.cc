...
NS_LOG_COMPONENT_DEFINE ("lbt_test");
/*
static void
...
*/
  ...
  switch (channelAccessManager)
    ...
    case Lbt:
    	...
    	//Config::SetDefault ("ns3::BasicLbtAccessManager::TxTime", TimeValue (Seconds (txTime/1000.0)));
//    	Config::SetDefault ("ns3::BasicLbtAccessManager::WaitTime", TimeValue (Seconds (150.0/1000.0)));
 ...
 // lbtAccessManag.m_backoffStartTime(TimeValue (MilliSeconds (80)));
  ...
  bsNodesA.Create (1);
  bsNodesB.Create (1);
  ueNodesA.Create (txTime);
  ueNodesB.Create (1);
  allWirelessNodes = NodeContainer (bsNodesA, bsNodesB, ueNodesA, ueNodesB);

  ns3::RngSeedManager::SetSeed(10000);
  	Ptr<UniformRandomVariable> randomNumber1, randomNumber2, randomNumber3, randomNumber4 = CreateObject<UniformRandomVariable> ();
  	unsigned int seed1, seed2, seed3, seed4;
  	std::stringstream ss;
  	ss << std::hex << randomNumber1;
  	ss >> seed1;

  	std::stringstream ss2;
  	  	ss2 << std::hex << randomNumber2;
  	  	ss2 >> seed2;

  	  std::stringstream ss3;
  	    	ss3 << std::hex << randomNumber3;
  	    	ss3 >> seed3;

  	    	std::stringstream ss4;
  	    	  	ss4 << std::hex << randomNumber4;
  	    	  	ss4 >> seed4;


  Ptr<ListPositionAllocator> positionAlloc = CreateObject<ListPositionAllocator> ();
  positionAlloc->Add (Vector (0.0, 0.0, 0.0));   // eNB1/AP in cell 0
  positionAlloc->Add (Vector (0.0, 0.0, 0.0)); // AP in cell 1
  positionAlloc->Add (Vector (0.0, 0.0, 5.0));  // UE1/STA in cell 0
  positionAlloc->Add (Vector (0.0, 0.0, 5000000000.0));  // STA in cell 1
